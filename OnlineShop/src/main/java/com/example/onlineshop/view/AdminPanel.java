...
public class AdminPanel {
    ...
    private void showConsumers() {
        ...
        while (!exit) {
            ...
            if (page != pageCount && page != 1) {
                System.out.println("1.Last page\t2.Next page\t3.quit\nTo award a user with a discount code enter their username , the type of discount(D C S V G) , percentage and capacity: ");
                String pageCommand = input.nextLine();
                if (pageCommand.matches("\\d")) {
                    switch (pageCommand) {
                        case "1":
                            page--;
                            break;
                    ...
                    String[] commands=pageCommand.split(" ");
                    boolean added=adminController.addDiscountCode(commands[0],Integer.parseInt(commands[1]),Double.parseDouble(commands[2]));
                    if(added){
            ...
            if (page == 1 && page != pageCount) {
                System.out.println("1.Next page\t2.quit\nTo award a user with a discount code enter their username , the type of discount(D=1 C=2 S=3 V=4) and percentage: ");
                String pageCommand = input.nextLine();
                if (pageCommand.matches("\\d")) {
                    switch (pageCommand) {
                        case "1": {
