...
public class ConsumerPanel {
    ...
    ConsumerPanel(ArrayList<Consumer> consumers, ArrayList<Commodity> commodities, ArrayList<Comment> comments, ArrayList<SignupRequest> signupRequests, ArrayList<CommentRequest> commentRequests, ArrayList<ChargeRequest> chargeRequests, Consumer consumer, ArrayList<String> usernames, ArrayList<String> emails, ArrayList<String> phoneNumbers) {
    ...
    public void consumerPage() {
        ...
        switch (command1) {
            ...
            case 8: {
                System.out.println("Your current discount codes are: ");
                System.out.println(consumerController.getDiscountCodes(consumer.getUsername()));
            }
            break;
            case 9: {
                exit = true;
    ...
    private void showProfile() {
        System.out.println(consumer.toString() + "\n1.Change username\t2.Change password\t3.Change email\t4.Change phone number");
    ...
    private void finalizePurchase() {
        System.out.println("Do you wish to use any discount codes");
        String command = "";
        while (!command.equals("1")) {
            System.out.print("Enter discount code:(to exit enter 1)");
            command = input.nextLine();
            consumerController.addDiscountCode(command, consumer.getUsername());
        }
        for (DiscountCode discountCode : consumer.getActivatedDiscountCodes()) {
            for (int i = 0; i < consumer.getCart().size(); i++) {
                if (discountCode.getCategory().equals(consumer.getCart().get(i).getCategory())) {
                    double price = consumer.getCartPrices().get(i);
                    consumer.getCartPrices().set(i, price * (100 - discountCode.getDiscountPercentage()) / 100);
                }
            }
        }
        for (int i = 0; i < consumer.getCart().size(); i++) {
            if (consumer.getCart().get(i).isDiscounted()&&consumer.getCart().get(i).getDiscountCap()>0) {
                double price = consumer.getCartPrices().get(i);
                consumer.getCartPrices().set(i, price * (100 - consumer.getCart().get(i).getDiscount()) / 100);
            }
        }
        double sumPaid = 0;
        ...
        if (purchaseMade) {
            ...
            Receipt receipt = new Receipt(LocalDateTime.now().toString(), consumer.getCart(), sumPaid);
    ...
    public String consumerCommands() {
        ...
        commands.append("\t6.Finalize purchase\t7.Shopping history\t8.View discount codes\t9.Exit");
